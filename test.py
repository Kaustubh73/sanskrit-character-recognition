...
class HandwritingDataCollector:
    def __init__(self, root):
        self.root = root
        self.root.title("Sanskrit Vowel Data Collector")
        ...
        self.current_char = list(self.vowels.keys())[0]
    ...
    def save_character(self):
        char_folder = f"{self.save_directory}/{self.vowels[self.current_char]}"
        try:
        ...
        print(f"Saved character {self.vowels[self.current_char]} as {image_path}")
        self.clear_canvas()
        self.current_sample += 1
    ...
    def next_character(self):
        current_index = list(self.vowels.keys()).index(self.current_char)
        if current_index < len(self.vowels) - 1:
            self.current_char = list(self.vowels.keys())[current_index + 1]
